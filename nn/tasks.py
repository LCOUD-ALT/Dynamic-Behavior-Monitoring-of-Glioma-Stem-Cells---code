...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 nn.ConvTranspose2d,  DWConvTranspose2d, C3x, RepC3,  C2f_AKConv, AKConv,
                 RepConv,  nn.Conv2d):
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_AKConv):
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m == "detect":
            return "detect"
        if m in ('segment', 'segment_dysnakeconv', 'segment_dbb', 'rfasegment'):
            return "segment"
        if m in ('pose', 'pose_dbb', 'rfapose'):
            return "pose"
        if m == "obb":
            return "obb"
        else:
